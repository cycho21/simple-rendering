...
function removeListener() {
	document.getElementById("whisper-form").removeEventListener("keydown", enterFunction);
}
...
function addWhisperListener() {
	document.getElementById("whisper-form").addEventListener("keydown", enterFunction);
}
...
function sessionEnd() {
	...
	function() {
		window.location.replace("http://10.10.44.71:80/");
	});
};

function unsubscribeChatroom(chatroomid) {
	stompClient.unsubscribe('/chatrooms/' + chatroomid);
}
...
function connect() {
	var socket = new SockJS('http://10.10.44.71:80/websocket');
...
$( document ).ready(function(){
	connect();
	document.getElementById("chatroomname").addEventListener("keydown", function(e) {
	    if (e.keyCode == 13) { 
			e.preventDefault();
	        sendMessage(id);
	    }
